...
extension EmulatorBusTests {
    ...
    struct MinimalEmulatorTest {
        ...
        @Test("Full connection flow", .disabled("CoreBluetoothEmulator advertise/scan is flaky in sandboxed test environments; run manually when debugging emulator behavior"))
        func testFullConnection() async throws {
            ...
            // Start scanning first (some emulators only emit discoveries to active scans)
            print("--- Scanning for peripheral ---")
            let discoverTask = Task<UUID?, Never> {
                for await discovered in central.scanForPeripherals(
                    withServices: [serviceUUID],
                    timeout: 5.0
                ) {
                    let name = discovered.name ?? discovered.advertisementData.localName ?? "unknown"
                    print("Discovered: \(name) with ID: \(discovered.id)")
            ...
            // Start advertising after scanning has begun
            ...
            let discoveredID = await discoverTask.value
            await central.stopScan()
            print("Scan complete")

            guard let peripheralID = discoveredID else {
                print("ERROR: Failed to discover peripheral")
                throw BleuError.connectionFailed("Failed to discover peripheral named \(peripheralName)")
            }
